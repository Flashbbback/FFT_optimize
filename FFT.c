...
void signal_gen(float real[], float imag[], int N) {
    for (int n = 0; n < N; n++) {
        real[n] = sinf(2 * M_PI * 10* n  / N)/N+sinf(2 * M_PI * 5 * n / N)/N;
        imag[n] = 0.0f;
    }
}
...
void signal_gen_q15(int16_t real[], int16_t imag[], int N) {
    for (int n = 0; n < N; n++) {
        ...
        float sin_val = sinf(2 * M_PI * n * 10 / N)/N+sinf(2 * M_PI * n * 3 / N)/N;
...
void fft_AVX(float real[], float imag[], int N, FFTContext *ctx)
{
    ...
    for (int s = 1;s<=m;s++)
    {
        ...
        {
            // w_real: 1.0, SQRT2_2, 0.0, -SQRT2_2 (重复两次)
            static const __m256 w_real = {
                    1.0f,  SQRT2_2,  0.0f, -SQRT2_2, 
                    -1.0f, -SQRT2_2,  0.0f, SQRT2_2
            };

            // w_imag: 0.0, -SQRT2_2, -1.0, -SQRT2_2 (重复两次)
